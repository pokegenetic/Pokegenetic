interface MoveData {
  name: string;
  type?: string;
  // Add other properties of a move if needed, e.g., power, accuracy
  [key: string]: any; 
}
...
const Edicion: React.FC = () => {
  ...
  const getMoveId = useCallback((moveNameOrId: string): string => {
    if (Moves[moveNameOrId] && typeof (Moves[moveNameOrId] as MoveData).name === 'string') return moveNameOrId; // It's already a valid ID with a name property
    const moveEntry = Object.entries(Moves as Record<string, MoveData>).find(([, moveDataObject]) => moveDataObject.name?.toLowerCase() === moveNameOrId?.toLowerCase());
  ...
  function capitalizeMoveName(moveId: string): string {
    return (Moves[moveId] as MoveData)?.name || moveId;
  }
  ...
  const handleSave = () => {
    const textToSave = generateShowdownText();
    // 1. Obtener el equipo actual de localStorage
    let team = getPokemonTeam();
    // 2. Determina si estamos editando un Pokémon existente o añadiendo uno nuevo.
    ...
    // Create a new Pokemon object from current state if we're not editing
    const pokemonToSave = parsedPokemon || {
      name: nickname || (initialPokemon?.name || entry?.name || "Unknown"),
      species: entry?.name || (initialPokemon?.name || "Unknown"),
      item: chosenItem || undefined,
      ability: chosenAbility || (entry?.abilities?.[0] || "Pressure"),
      ...
      teraType: teraTypeMap[chosenTera] || undefined,
      types: entry?.types || []
    };
    ...
    // 3. Guarda el texto Showdown del equipo completo y actualizado en localStorage (solo para exportar)
    const finalTeam = getPokemonTeam();
    localStorage.setItem('pokemonTeamShowdownText', finalTeam.map(convertirAShowdown).join("\n\n"));
    
    // 4. Navega a la página del equipo. equipo.tsx carga desde localStorage
    navigate('/equipo');
    playSaveSound();
  };
