...
import { signInWithPopup, GoogleAuthProvider, sendSignInLinkToEmail, signOut, onAuthStateChanged } from 'firebase/auth';
import { doc, onSnapshot } from 'firebase/firestore';
import { saveUserProfile, getUserProfile, updateUserFichas } from '../lib/userData';
...
export const UserProvider = ({ children }) => {
    ...
    const createOrUpdateFirebaseProfile = async (firebaseUser) => {
        try {
    ...
    const syncPokeballsWithFirebase = async (pokeballs) => {
        if (!user?.uid || !user?.email)
            return;
        try {
    ...
    const syncIncubadorasWithFirebase = async (incubadoras) => {
        if (!user?.uid || !user?.email) {
    ...
    const syncFichasWithFirebase = async (fichas) => {
        if (!user?.uid || !user?.email)
            return;
        try {
    ...
    const updatePokeballsInContext = async (newPokeballs) => {
        if (user?.profile) {
    ...
    useEffect(() => {
        const handleFichasUpdate = (event) => {
