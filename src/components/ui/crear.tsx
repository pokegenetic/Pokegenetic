...
import { parseShowdownText, ParsedPokemon, generateShowdownTextFromParsed } from "../../lib/parseShowdown";
...
export default function Crear() {
  ...
  const [activeTab, setActiveTab] = useState<'crear' | 'showdown' | 'packs'>('crear');
  ...
  const renderTransferOnlyWarning = () => {
    if (!isLegal) {
      return <div className="warning">Este Pokémon es transfer-only y no puede ser creado.</div>;
    }
  };
  ...
  const validatePokemonLegality = (pokemon: Pokemon, shiny: boolean) => {
  ...
  const handleCreate = () => {
    ...
    if (!validatePokemonLegality(selectedPokemon, shiny)) return;
  ...
  function handleCreateShowdown(parsedEntries: Array<{ pokemon: ParsedPokemon, messages: string[] }>) {
  ...
  return (
    <div className={
      `flex flex-col items-center px-4 pt-2 text-center${isSearchBarFocused ? ' pb-72' : ''}`
    }>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 pointer-events-none">
          <div className="flex flex-col items-center animate-fade-in-down">
            <div className="relative">
              <img src="/pokeball.png" alt="Pokeball" className="w-24 h-24 animate-bounce" />
              <span className="absolute right-0 bottom-0 bg-green-500 text-white rounded-full p-2 shadow-lg text-3xl border-4 border-white">✔</span>
            </div>
            <span className="mt-2 text-lg font-bold text-green-600 drop-shadow-lg">¡Importado con éxito!</span>
          </div>
        </div>
      )}

      {/* Tabs - estilo replicado de paquetes.tsx y tamaño igualado */}
      <div className="flex justify-center gap-2 mb-6">
        <button
          className={`w-28 text-sm px-2 py-2 rounded-t-lg font-semibold transition-colors border-b-2 ${activeTab === 'crear' ? 'bg-white/80 dark:bg-gray-900/80 border-yellow-400 text-yellow-600' : 'bg-gray-200/70 dark:bg-gray-700/70 border-transparent text-gray-700 dark:text-gray-300'}`}
          onClick={() => { playSoundEffect('notification', 0.13); setActiveTab('crear'); }}
        >
          <span>Crear</span>
          <span className="block text-[11px] font-normal">Pokémon</span>
        </button>
        <button
          className={`w-28 text-sm px-2 py-2 rounded-t-lg font-semibold transition-colors border-b-2 ${activeTab === 'showdown' ? 'bg-white/80 dark:bg-gray-900/80 border-blue-400 text-blue-600' : 'bg-gray-200/70 dark:bg-gray-700/70 border-transparent text-gray-700 dark:text-gray-300'}`}
          onClick={() => { playSoundEffect('notification', 0.13); setActiveTab('showdown'); }}
        >
          <span>Importar</span>
          <span className="block text-[11px] font-normal">Showdown</span>
        </button>
        <button
          className={`w-28 text-sm px-2 py-2 rounded-t-lg font-semibold transition-colors border-b-2 ${activeTab === 'packs' ? 'bg-white/80 dark:bg-gray-900/80 border-pink-400 text-pink-600' : 'bg-gray-200/70 dark:bg-gray-700/70 border-transparent text-gray-700 dark:text-gray-300'}`}
          onClick={() => { playSoundEffect('notification', 0.13); setActiveTab('packs'); }}
        >
          <span>Packs</span>
          <span className="block text-[11px] font-normal">y Eventos</span>
        </button>
      </div>
      ...
      {activeTab === 'crear' && (
        <>
          <div className="flex items-center justify-center w-full my-2">
            ...
            <img src="/pokeball.png" alt="Pokeball" className="mx-2 w-6 h-6" />
            <hr className="flex-grow border-t border-gray-300" />
          </div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mb-2">
            Ingresa el nombre del Pokémon que quieres crear en la barra de búsqueda
          </p>

          <div className={`w-full max-w-xs sm:max-w-md flex flex-col justify-start mt-2 ${selectedPokemon ? 'min-h-[220px]' : 'min-h-[60px]'}`}> 
           {selectedPokemon && (
              <div className="flex flex-col items-center justify-center mb-2 min-h-[160px]">
                <div className="pokeball-flash-anim-container" key={pokeAnimKey} style={{ width: 140, height: 140 }}>
                  {/* Pokéball destello animado */}
                  <img src="/pokeball.png" alt="Pokeball" className="pokeball-flash" draggable={false} style={{ width: 70, height: 70 }} />
                  {/* Sprite con animación pop y click para reproducir cry y animar */}
                  <img
                    src={getSpriteForSelectedPokemon(selectedPokemon)}
                    alt={selectedPokemon.name}
                    className="pokemon-sprite-pop"
                    draggable={false}
                ...
                <div className="flex flex-col items-center mt-1">
                  <h3 className="text-gray-800 font-bold capitalize text-lg sm:text-2xl mb-0.5">{selectedPokemon.name}</h3>
                  <div className="flex items-center space-x-2">
                    <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full">
                      #{selectedPokemon.num}
                    </span>
                    <span className="px-2 py-0.5 bg-gray-200 text-gray-600 text-xs rounded-full">
                      Gen {getGeneration(selectedPokemon.num)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <SearchBar
              ...
              onFocus={() => { handleSearchBarFocus(); setIsDropdownOpen(true); }}
              onBlur={() => { handleSearchBarBlur(); setIsDropdownOpen(false); }}
      ...
      {activeTab === 'showdown' && (
        <>
          ...
          <h2 className="text-2xl sm:text-3xl font-bold mb-4 pb-1 bg-gradient-to-r from-yellow-400 via-pink-500 to-blue-500 bg-clip-text text-transparent drop-shadow-xl animate-fade-in-down text-center">
            Importar desde Showdown
          </h2>
          <p className="text-xs sm:text-sm text-gray-500 mb-2">
            Pega aquí el texto de tu equipo exportado desde Pokémon Showdown
          </p>
          <ShowdownTextarea
            placeholder="Pega aquí el texto de tus Pokémon"
            className="w-full max-w-xs sm:max-w-md mb-0"
            onFocus={handleShowdownTextareaFocus}
            onBlur={handleShowdownTextareaBlur}
      ...
      {activeTab === 'packs' && (
        <>
          ...
          <div className="mb-6" />
          <div className="flex flex-row gap-4 max-w-2xl mx-auto mb-10 justify-center items-center">
            <button
              onClick={() => {
                playSoundEffect('notification', 0.1);
                navigate('/paquetes');
              }}
              className="flex items-center justify-center px-4 py-2.5 rounded-full bg-gradient-to-r from-yellow-400 via-pink-500 to-blue-500 text-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-opacity-50 w-[170px] h-[60px]"
            >
              <div className="flex flex-col items-center">
                <span className="font-bold text-sm">Packs</span>
                <span className="text-xs text-white/80">Colecciones</span>
              </div>
            </button>
            <button
              onClick={() => {
              ...
              className="flex items-center justify-center px-4 py-2.5 rounded-full bg-gradient-to-r from-yellow-400 via-pink-500 to-blue-500 text-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-opacity-50 w-[175px] h-[60px]"
            >
              <div className="flex flex-col items-center">
                <span className="font-bold text-sm">Eventos</span>
                <span className="text-xs text-white/80">Los más buscados</span>
              </div>
            </button>
          </div>
        </>
...
function getSpriteForSelectedPokemon(pokemon: Pokemon): string {
...
export function getSpriteForPokemon(pokemon: { name: string; sprite?: string }): string {
  const name = pokemon.name.toLowerCase();
