...
const BattleSystem = ({ gimnasioActual, entrenadorActual, pokemonActual, combatienteActual, tipoSeleccionado, gimnasios, onVictoria, onDerrota, onVolverAlGimnasio, estado }) => {
    ...
    // Estados para controlar el debounce y ventajas táctiles
    ...
    // Función de tap mejorada con ventajas para móviles
    const processTap = (isTouch = false) => {
        if (!combateEnCurso)
            return;
        ...
        // VENTAJAS PARA DISPOSITIVOS TÁCTILES
        let tapMultiplier = 1;
        let criticoChance = 0.02; // 2% base
        if (isTouch && isTouchDevice) {
            // Móviles tienen ventajas significativas
            tapMultiplier = 1.5; // 50% más efectivo
            criticoChance = 0.05; // 5% de crítico (2.5x más que PC)
        }
        // Calcular si es un ataque crítico
        const esCritico = Math.random() < criticoChance;
        if (esCritico) {
            // Ataque crítico con ventaja móvil
            playSoundEffect('notification', 0.3);
            const criticoValue = isTouch ? 6 : 4; // Móviles hacen más daño crítico
            setTapsAcumulados(prev => prev + criticoValue);
            setTapsHistoricos(prev => prev + criticoValue);
            // Mostrar mensaje de crítico
            setMensajeCritico(isTouch ? '¡SÚPER CRÍTICO MÓVIL!' : '¡GOLPE CRÍTICO!');
            ...
            // Ataque normal con multiplicador móvil
            playSoundEffect('pop', 0.1);
            const normalValue = Math.ceil(1 * tapMultiplier);
            setTapsAcumulados(prev => prev + normalValue);
            setTapsHistoricos(prev => prev + normalValue);
        }
    };
    const handleTap = () => {
        processTap(false); // Click de PC
    };
    ...
    const handleTouchStart = (e) => {
        ...
        const handleTouchStart = (e) => {
            ...
            if (now - lastTapTime < TAP_DEBOUNCE_MS)
                return;
            // Registrar el toque con ventajas móviles
            processTap(true); // Touch de móvil
        };
        ...
        // Función mejorada para SuperTap con ventajas móviles
        const processSuperTap = (isTouch = false) => {
            ...
            // VENTAJAS PARA DISPOSITIVOS TÁCTILES en SuperTap
            let superValue = 20; // Base para PC
            if (isTouch && isTouchDevice) {
                superValue = 30; // 50% más poderoso en móviles
            }
            // Ataque Cargado con ventaja móvil
            playSoundEffect('superpower', 0.9);
            setTapsAcumulados(prev => prev + superValue);
            setTapsHistoricos(prev => prev + superValue);
            // Mostrar mensaje de Ataque Cargado
            setMensajeCritico(isTouch ? '¡SÚPER CARGA MÓVIL!' : '¡ATAQUE CARGADO!');
        ...
        const handleSuperTap = () => {
            processSuperTap(false); // Click de PC
        };
        ...
        const handleSuperTouchStart = (e) => {
            ...
            if (now - lastSuperTapTime < TAP_DEBOUNCE_MS)
                return;
            // Registrar el super toque con ventajas móviles
            processSuperTap(true); // Touch de móvil
        };
        ...
        } // Esta llave cierra el if de faseAnimacion
        ...
        return (_jsxs("div", { className: "fixed inset-0 bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 flex flex-col items-center justify-center z-[60] text-white", children: [_jsx("div", { className: "absolute inset-0 bg-black/30" }), _jsxs("div", { className: "relative z-10 flex flex-col items-center justify-center w-full h-full px-4", children: [_jsx("div", { className: "flex items-center gap-4 mb-8 bg-black/40 backdrop-blur-sm rounded-xl p-4 border border-white/20", children: _jsxs("div", { className: "text-shadow-lg", children: [_jsx("h2", { className: "text-2xl font-bold text-white drop-shadow-lg", children: oponenteNombre }), _jsx("p", { className: "text-lg text-gray-100 drop-shadow-md", children: pokemonNombre }), _jsxs("p", { className: "text-sm text-gray-200 drop-shadow-md", children: ["Tipo: ", gimnasio.tipo] })] }) }), _jsx("div", { className: "mb-6", children: _jsx("div", { className: "relative bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20", children: _jsx("img", { src: getPokemonIconUrl(pokemonNombre), alt: pokemonNombre, className: "w-32 mx-auto", style: {
                                        ...
                                        calcularEfectividad(tipoSeleccionado, gimnasio.tipo) === 'debil' ? 'No muy efectivo' :
                                            'Efectividad normal' })] }))] })] }));
    }; // Esta llave cierra la función BattleSystem
