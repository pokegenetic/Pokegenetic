...
export function PokeballsDisplay({ balls: externalBalls, subscribe, onSelectBall, selectedBallKey }: { balls?: any, subscribe?: (cb: () => void) => void, onSelectBall?: (key: string) => void, selectedBallKey?: string } = {}) {
  const { user } = useUser();
  ...
  useEffect(() => {
    if (externalBalls) setBalls(externalBalls);
  }, [externalBalls]);
  ...
  return (
    <div className="flex flex-row gap-4 justify-center items-end mt-2" style={{ background: 'rgba(255,255,255,0.35)', borderRadius: 12, padding: 8 }}>
      {BALL_TYPES.map(ball => {
        ...
        return (
          <div
            key={ball.key}
            className={`flex flex-col items-center ${available ? 'pokeball-selectable' : 'pokeball-disabled'}${isSelected ? ' ring-2 ring-yellow-400' : ''}`}
            onClick={() => available && onSelectBall && onSelectBall(ball.key)}
            style={{
              borderRadius: 8,
          ...
          >
            <img
              ...
              className={shakeKeys[ball.key] && available ? 'pokeball-gentle-shake' : ''}
            />
            <span className="text-xs font-semibold whitespace-nowrap text-center w-full block mt-1" style={{ color: ball.color, lineHeight: 1.1 }}>{ball.label}</span>
            <span className="text-sm font-bold text-gray-800 mt-0.5">{balls[ball.key]}</span>
          </div>
        );
      })}
    </div>
