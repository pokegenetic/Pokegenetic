...
const AnimatedNavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
  ...
  return (
    ...
    >
      ...
      {/* Indicador de página activa - barra de degradado */}
      {isActive && (
    ...
    <div className="relative ml-3">
      ...
      >
        {selectedGame && selectedGame.id && gameImages[selectedGame.id] ? (
          ...
          <p className="text-xs sm:text-sm font-semibold text-gray-700 px-2">🎮 Sin juego seleccionado ▼</p>
        )}
      </div>
      {dropdownOpen && (
  ...
  const handleExportTeam = async () => {
    ...
    if (!user) {
      const shouldLogin = window.confirm(
        'Para enviar tu equipo al laboratorio necesitas iniciar sesión.\n\n' +
        'Esto nos permite:\n' +
    ...
    <div className="relative group w-fit sm:w-auto max-w-full">
      ...
      <button
        ...
        onClick={teamCount > 0 ? handleExportTeam : () => {
          if (!user) {
            const shouldLogin = window.confirm(
              'Para enviar equipos al laboratorio necesitas iniciar sesión.\n\n' +
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-20 w-full px-4 sm:px-6 py-3 backdrop-blur-md bg-white/20 shadow-md transition-all duration-200 ease-in-out">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
          ...
          <div className="relative flex flex-col items-center">
            ...
            {showAudioNotice && (
              ...
              >
                ...
                ></span>
                Toca el ícono de música para
                activar el audio de fondo 🎵
              </div>
            )}
          </div>
          {selectedGameDisplay}
        ...
        {/* Botón Enviar al Laboratorio y UserMenu */}
