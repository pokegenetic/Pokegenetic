...
// Event-specific validation data
export interface EventPokemonData {
	species: string;
	level: number;
	shininess: 'locked' | 'forced' | 'allowed';
	requiredIVs?: Partial<Record<'hp' | 'atk' | 'def' | 'spa' | 'spd' | 'spe', number>>;
...
export const EVENT_POKEMON_DISTRIBUTIONS = new Map<string, EventPokemonData[]>([
	['mewtwo', [{
		species: 'mewtwo',
		level: 100,
		shininess: 'locked',
...
export class LegalityAnalysis {
	private results: LegalityResult[] = [];
	private generation: number;
	private dex: any; // Dex instance

	constructor(generation: number, dex: any) {
		this.generation = generation;
		this.dex = dex;
	}

	// Main validation method
	public validatePokemon(set: ParsedPokemon): LegalityResult[] {
		this.results = [];
	...
	private verifySpeciesLegality(species: SpeciesData, set: ParsedPokemon): void {
		// Check for banned legendary categories
		if (species.tags?.includes('Mythical')) {
			this.addResult(LegalityResultType.Invalid, 
				`${species.name} is a Mythical Pokémon and may be restricted in this format`, 
				'LegalityMythical', 8);
		}
		
		if (species.tags?.includes('Restricted Legendary')) {
			this.addResult(LegalityResultType.Invalid, 
				`${species.name} is a Restricted Legendary and may be banned in this format`, 
				'LegalityRestrictedLegendary', 9);
		}
		
		if (species.tags?.includes('Ultra Beast')) {
			this.addResult(LegalityResultType.Fishy, 
				`${species.name} is an Ultra Beast with special encounter requirements`, 
				'LegalityUltraBeast', 3);
		}
	}

	// Level restrictions verifier
	private verifyLevelRestrictions(set: ParsedPokemon): void {
		const constraints = GENERATION_CONSTRAINTS[this.generation as keyof typeof GENERATION_CONSTRAINTS];
		if (!constraints) return;

		if (set.level < constraints.minLevel) {
			this.addResult(LegalityResultType.Invalid, 
				`Level ${set.level} is below minimum level ${constraints.minLevel} for Gen ${this.generation}`, 
				'LegalityLevelTooLow', 7);
		}
		
		if (set.level > constraints.maxLevel) {
			this.addResult(LegalityResultType.Invalid, 
				`Level ${set.level} exceeds maximum level ${constraints.maxLevel} for Gen ${this.generation}`, 
				'LegalityLevelTooHigh', 7);
		}
	}

	// Paradox Pokémon level restrictions (Area Zero minimum level 55)
	private verifyParadoxPokemonLevels(species: SpeciesData, set: ParsedPokemon): void {
		...
		if (minLevel && set.level < minLevel) {
			this.addResult(LegalityResultType.Invalid, 
				`${species.name} cannot be obtained below level ${minLevel} (Area Zero restriction)`, 
				'LegalityParadoxMinLevel', 8);
		}
	}

	// Move legality verifier
	private verifyMoveLegality(species: SpeciesData, set: ParsedPokemon): void {
		if (!set.moves || set.moves.length === 0) return;
		...
		for (const moveId of set.moves) {
			...
			if (!move || !move.exists) {
				this.addResult(LegalityResultType.Invalid, 
					`Move ${moveId} does not exist`, 
					'LegalityMoveNonexistent', 8);
				continue;
			}
			...
			if (move.gen > this.generation) {
				this.addResult(LegalityResultType.Invalid, 
					`${move.name} is not available in Gen ${this.generation}`, 
					'LegalityMoveFutureGen', 8);
			}
	...
	private verifyShinyLegality(species: SpeciesData, set: ParsedPokemon): void {
		if (!set.isShiny) return; // Not shiny, no restrictions

		const speciesId = this.dex.toID(species.name);
		if (SHINY_LOCKED_POKEMON.has(speciesId)) {
			this.addResult(LegalityResultType.Invalid, 
				`${species.name} is shiny-locked and cannot be legitimately shiny`, 
				'LegalityShinyLocked', 9);
		}
	...
	private verifyEventPokemonLegality(species: SpeciesData, set: ParsedPokemon): void {
		const speciesId = this.dex.toID(species.name);
		const eventDistributions = EVENT_POKEMON_DISTRIBUTIONS.get(speciesId);
		
		if (!eventDistributions) return; // Not an event Pokémon
		...
		for (const event of eventDistributions) {
			...
			if (event.shininess === 'locked' && set.isShiny) {
				this.addResult(LegalityResultType.Invalid, 
					`${species.name} from ${event.eventName} cannot be shiny`, 
					'LegalityEventShinyLocked', 9);
				eventMatch = false;
			} else if (event.shininess === 'forced' && !set.isShiny) {
				this.addResult(LegalityResultType.Invalid, 
					`${species.name} from ${event.eventName} must be shiny`, 
					'LegalityEventShinyForced', 9);
				eventMatch = false;
			}
			...
			if (event.requiredIVs && set.IVs) {
				for (const [stat, requiredIV] of Object.entries(event.requiredIVs)) {
					if (set.IVs[stat as keyof typeof set.IVs] !== requiredIV) {
						this.addResult(LegalityResultType.Invalid, 
							`${species.name} from ${event.eventName} must have ${stat.toUpperCase()} IV of ${requiredIV}`, 
							'LegalityEventIVMismatch', 8);
						eventMatch = false;
					}
				}
			}
			...
			if (event.requiredAbility && set.ability && 
				...
				this.addResult(LegalityResultType.Invalid, 
					`${species.name} from ${event.eventName} must have ${event.requiredAbility} ability`, 
					'LegalityEventAbilityMismatch', 8);
				eventMatch = false;
			}
			
			if (eventMatch) {
				matchesEvent = true;
				this.addResult(LegalityResultType.Valid, 
					`${species.name} matches ${event.eventName} distribution`, 
					'LegalityEventMatch', 0);
				break;
			}
		}
		...
		if (!matchesEvent) {
			this.addResult(LegalityResultType.Fishy, 
				`${species.name} does not match any known event distribution`, 
				'LegalityEventNoMatch', 5);
		}
	}

	// Ability legality verifier
	private verifyAbilityLegality(species: SpeciesData, set: ParsedPokemon): void {
		if (!set.ability) return;

		const ability = this.dex.abilities.get(set.ability);
		if (!ability || !ability.exists) {
			this.addResult(LegalityResultType.Invalid, 
				`Ability ${set.ability} does not exist`, 
				'LegalityAbilityNonexistent', 8);
			return;
		}
		...
		const hasAbility = Object.values(abilities).some(abilityName => 
			this.dex.toID(abilityName) === ability.id
		);

		if (!hasAbility) {
			this.addResult(LegalityResultType.Invalid, 
				`${species.name} cannot have the ability ${ability.name}`, 
				'LegalityAbilityMismatch', 8);
		}
		...
		if (abilities.H && this.dex.toID(abilities.H) === ability.id) {
			...
			if (this.generation < 5) {
				this.addResult(LegalityResultType.Invalid, 
					`Hidden Abilities are not available before Gen 5`, 
					'LegalityHiddenAbilityGen', 7);
			}
		}
	}

	// Ball legality verifier
	private verifyBallLegality(species: SpeciesData, set: ParsedPokemon): void {
		if (!set.item) return; // ParsedPokemon doesn't have pokeball property, using item
		...
		if (!ball || !ball.exists || !ball.isPokeball) {
			...
			if (set.item && set.item.toLowerCase().includes('ball')) {
				this.addResult(LegalityResultType.Invalid, 
					`${set.item} is not a valid Poké Ball`, 
					'LegalityInvalidBall', 6);
			}
			return;
		}
		...
		if (ball.gen > this.generation) {
			this.addResult(LegalityResultType.Invalid, 
				`${ball.name} is not available in Gen ${this.generation}`, 
				'LegalityBallFutureGen', 7);
		}

		// Special ball restrictions
		if (ball.id === 'masterball') {
			this.addResult(LegalityResultType.Fishy, 
				`Master Ball usage should be verified for legitimacy`, 
				'LegalityMasterBall', 2);
		}

		if (ball.id === 'cherishball') {
			this.addResult(LegalityResultType.Fishy, 
				`Cherish Ball indicates event Pokémon - verify event data`, 
				'LegalityCherishBall', 3);
		}
	}

	// Item legality verifier
	private verifyItemLegality(set: ParsedPokemon): void {
		if (!set.item) return;

		const item = this.dex.items.get(set.item);
		if (!item || !item.exists) {
			this.addResult(LegalityResultType.Invalid, 
				`Item ${set.item} does not exist`, 
				'LegalityItemNonexistent', 7);
			return;
		}
		...
		if (item.gen > this.generation) {
			this.addResult(LegalityResultType.Invalid, 
				`${item.name} is not available in Gen ${this.generation}`, 
				'LegalityItemFutureGen', 7);
		}

		// Check for banned items
		if (item.isNonstandard) {
			this.addResult(LegalityResultType.Invalid, 
				`${item.name} is not available in standard play`, 
				'LegalityItemNonstandard', 8);
		}
	}

	// IV legality verifier
	private verifyIVLegality(set: ParsedPokemon): void {
		const constraints = GENERATION_CONSTRAINTS[this.generation as keyof typeof GENERATION_CONSTRAINTS];
		if (!constraints || !set.IVs) return;

		const ivStats = ['hp', 'atk', 'def', 'spa', 'spd', 'spe'] as const;
		
		for (const stat of ivStats) {
			const iv = set.IVs[stat];
			if (iv !== undefined && iv > constraints.maxIV) {
				this.addResult(LegalityResultType.Invalid, 
					`${stat.toUpperCase()} IV ${iv} exceeds maximum ${constraints.maxIV} for Gen ${this.generation}`, 
					'LegalityIVTooHigh', 8);
			}
			
			if (iv !== undefined && iv < 0) {
				this.addResult(LegalityResultType.Invalid, 
					`${stat.toUpperCase()} IV ${iv} cannot be negative`, 
					'LegalityIVNegative', 8);
			}
		}
		...
		if (perfectIVs === 6 && this.generation <= 3) {
			this.addResult(LegalityResultType.Fishy, 
				`Perfect IVs in all stats are extremely rare in Gen ${this.generation}`, 
				'LegalityPerfectIVs', 2);
		}
	}

	// Helper method to add results
	private addResult(type: LegalityResultType, message: string, identifier: string, severity: number): void {
		this.results.push({
			type,
			message,
			identifier,
	...
	public getOverallStatus(): LegalityResultType {
	...
	public getMaxSeverity(): number {
		return this.results.reduce((max, result) => Math.max(max, result.severity), 0);
	}

	// Filter results by type
	public getResultsByType(type: LegalityResultType): LegalityResult[] {
...
export class AbilityVerifier {
	static verifyAbilityChange(fromAbility: string, toAbility: string, hasAbilityCapsule: boolean, hasAbilityPatch: boolean, generation: number): LegalityResult | null {
...
export class BallVerifier {
	static verifyBallInheritance(species: SpeciesData, parentBalls: string[], generation: number): LegalityResult | null {
		const inheritanceRules = BALL_INHERITANCE_RULES[generation as keyof typeof BALL_INHERITANCE_RULES];
		if (!inheritanceRules) return null;
...
export class MiscVerifier {
	static verifyNickname(nickname: string, species: SpeciesData): LegalityResult | null {
	...
	static verifyOT(otName: string, generation: number): LegalityResult | null {
		if (!otName) return null;
...
export function validatePokemonLegality(set: ParsedPokemon, generation: number, dex: any): LegalityResult[] {
...
export function isShinyLocked(species: string): boolean {
...
export function getParadoxMinLevel(species: string): number | null {
	const speciesId = species.toLowerCase().replace(/[^a-z0-9]/g, '');
	return PARADOX_POKEMON_MIN_LEVELS.get(speciesId) || null;
}

// Helper function to validate against event distributions
export function validateEventPokemon(set: ParsedPokemon): {
	isEventPokemon: boolean;
	matchingEvents: EventPokemonData[];
	violations: string[];
} {
	...
	const matchingEvents: EventPokemonData[] = [];
	const violations: string[] = [];
	
	for (const event of eventDistributions) {
		const eventViolations: string[] = [];
		...
		if (event.requiredIVs && set.IVs) {
			for (const [stat, requiredIV] of Object.entries(event.requiredIVs)) {
				if (set.IVs[stat as keyof typeof set.IVs] !== requiredIV) {
					eventViolations.push(`${stat.toUpperCase()} IV mismatch: expected ${requiredIV}, got ${set.IVs[stat as keyof typeof set.IVs]}`);
				}
			}
		}
...
export function createLegalPokemon(
	species: string,
	options: {
		level?: number;
		shiny?: boolean;
...
export type { SpeciesData, ParsedPokemon };
