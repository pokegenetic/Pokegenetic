...
export async function saveLigaPokemonProgress(uid: string, email: string, medallasObtenidas: number[], entrenadoresVencidos: {[key: string]: boolean}) {
  const documentId = email || uid;
  try {
    await updateDoc(doc(db, 'users', documentId), {
      ligaPokemonProgress: {
        medallasObtenidas,
        entrenadoresVencidos,
        updatedAt: serverTimestamp()
      }
    });
    console.log('✅ Progreso Liga Pokémon guardado en Firestore:', { medallasObtenidas, entrenadoresVencidos });
  } catch (error) {
    console.error('❌ Error guardando progreso Liga Pokémon:', error);
    throw error;
  }
}

export async function getLigaPokemonProgress(uid: string, email?: string): Promise<{medallasObtenidas: number[], entrenadoresVencidos: {[key: string]: boolean}} | null> {
  const documentId = email || uid;
  try {
    const ref = doc(db, 'users', documentId);
    const snap = await getDocFromServer(ref);
    ...
    if (snap.exists()) {
      ...
      if (data.ligaPokemonProgress) {
        console.log('✅ Progreso Liga Pokémon cargado desde Firestore:', data.ligaPokemonProgress);
        return {
          medallasObtenidas: data.ligaPokemonProgress.medallasObtenidas || [],
          entrenadoresVencidos: data.ligaPokemonProgress.entrenadoresVencidos || {}
        };
      }
    }
    
    console.log('📝 No se encontró progreso Liga Pokémon en Firestore, iniciando desde cero');
    return null;
  } catch (error) {
    console.error('❌ Error cargando progreso Liga Pokémon:', error);
    return null;
  }
}
