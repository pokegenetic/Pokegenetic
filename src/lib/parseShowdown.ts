...
import { getGenderForSpecies, getRandomMovesForSpecies, getPokemonDetails, PokemonDetails } from './pokedexHelper';
import { genderlessPokemon } from '@/data/sv/genderless';
import { Moves } from '@/data/sv/moves';

// Define GenderName locally if it's not easily importable or part of PokemonDetails
type GenderName = "M" | "F" | "N";

export interface ParsedPokemon {
  name: string; // Nickname or species if no nickname
  species: string; // Actual species name
  gender?: GenderName;
  item?: string;
  ability: string | null; // MODIFIED: Allow null for ability
  isShiny: boolean;
  isHO?: boolean; // ADDED: Hidden Ability flag
  teraType: string;
...
export function parseShowdownText(text: string): { pokemon: ParsedPokemon, messages: string[] } {
  const lines = text.split('\n').map(l => l.trim()).filter(l => l);
  const autoAssignmentMessages: string[] = [];
  ...
  let nature: string = 'Serious';
  let moves: string[] = [];
  let teraType: string = '';
  let types: string[] | undefined = ['Normal'];
  // Default IVs
  ...
  if (genderMatch) {
    gender = genderMatch[1].toUpperCase() as GenderName;
  ...
  const pokemonDetails: PokemonDetails | null = getPokemonDetails(cleanSpeciesName);
  ...
  if (!pokemonDetails) {
    ...
    if (moves.length < 4) autoAssignmentMessages.push(`Assigned default/random moves for ${cleanSpeciesName} as it was not found or had no moves.`);
  } else {
    ...
    if (line.startsWith('Ability: ')) {
      ...
      evParts.forEach(part => {
        ...
        if (key && evs.hasOwnProperty(key)) {
          (evs as any)[key] = parseInt(valueStr, 10) || 0;
        }
      });
      ...
      ivParts.forEach(part => {
        ...
        if (key && ivs.hasOwnProperty(key)) {
          (ivs as any)[key] = parseInt(valueStr, 10) || 0;
        }
      });
...
export function parseShowdownTeam(text: string): Array<{ pokemon: ParsedPokemon, messages: string[] }> {
...
export function parseShowdownTextToArray(fullText: string): ParsedPokemon[] {
  if (!fullText || fullText.trim() === '') {
    return [];
  }
  const pokemonBlocks = fullText.split('\n\n');
  const parsedTeam: ParsedPokemon[] = [];
...
export async function readEquipoTxtAndParse(): Promise<ParsedPokemon[]> {
...
export async function writeEquipoTxtFromTeam(team: ParsedPokemon[]): Promise<void> {
  // Filtra Pokémon válidos antes de exportar
  const validTeam = team.filter(p =>
    p &&
    p.species &&
...
export async function addPokemonToEquipoTxt(newPokemon: ParsedPokemon): Promise<void> {
...
export async function updatePokemonInEquipoTxt(index: number, updatedPokemon: ParsedPokemon): Promise<void> {
  const team = await readEquipoTxtAndParse();
...
export async function deletePokemonFromEquipoTxt(index: number): Promise<void> {
  const team = await readEquipoTxtAndParse();
  if (index >= 0 && index < team.length) {
    team.splice(index, 1);
    await writeEquipoTxtFromTeam(team);
...
export function generateShowdownTextFromParsed(p: ParsedPokemon): string {
  ...
  if (p.EVs && Object.values(p.EVs).some(ev => typeof ev === 'number' && ev > 0)) {
    ...
    for (const stat of statsOrder) {
      const value = (p.EVs as any)[stat.key];
  ...
  if (p.IVs && Object.values(p.IVs).some(iv => typeof iv === 'number' && iv !== 31)) {
    ...
    for (const stat of statsOrder) {
      const value = (p.IVs as any)[stat.key];
