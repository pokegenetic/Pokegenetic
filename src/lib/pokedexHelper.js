 ...
 */
export function getPokemonDetails(speciesName) {
    const speciesId = speciesName.toLowerCase().replace(/[^a-z0-9]+/g, '');
    const speciesData = Pokedex[speciesId];
    if (!speciesData) {
 ...
 */
export function getGenderForSpecies(speciesName) {
    const speciesId = speciesName.toLowerCase().replace(/[^a-z0-9]+/g, '');
    const speciesData = Pokedex[speciesId];
    if (!speciesData) {
 ...
 */
export function getRandomMovesForSpecies(speciesName) {
    const speciesId = speciesName.toLowerCase().replace(/[^a-z0-9]+/g, '');
    const speciesData = Pokedex[speciesId];
    let availableMovesPool = [];
    if (speciesData) {
        if (speciesData.randomBattleMoves && speciesData.randomBattleMoves.length > 0) {
