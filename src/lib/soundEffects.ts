...
// Función para obtener la URL del sonido con sistema de fallback
...
// Función mejorada para reproducir audio con sistema de fallback automático
const playAudioWithFallback = async (soundType: string, volume: number = 1.0, loop: boolean = false): Promise<HTMLAudioElement | null> => {
    ...
    try {
        console.log(`🔊 Intentando reproducir "${soundType}" desde CDN: ${soundUrl}`);
        ...
        console.log(`✅ Sonido "${soundType}" reproducido desde CDN`);
        return audio;
        
    } catch (error) {
        console.warn(`⚠️ Error con CDN para "${soundType}":`, error);
        
        // Fallback a archivo local
        const localUrl = getSoundUrl(soundType, true);
        if (!localUrl) {
            console.error(`❌ No hay fallback local para "${soundType}"`);
            return null;
        }
        
        try {
            console.log(`🔄 Intentando fallback local para "${soundType}": ${localUrl}`);
            const localAudio = new Audio(localUrl);
            localAudio.volume = Math.max(0, Math.min(1, volume));
            localAudio.loop = loop;
            
            await localAudio.play();
            console.log(`✅ Sonido "${soundType}" reproducido desde archivo local`);
            return localAudio;
            
        } catch (localError) {
            console.error(`❌ Error también con archivo local para "${soundType}":`, localError);
            return null;
        }
    }
};
