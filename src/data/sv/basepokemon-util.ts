...
function isLegalPokemon(pokemon: BasePokemon): boolean {
...
export function parseShowdownEntry(entry: string): BasePokemon | null {
  ...
  let name = lines[0];
  let gender, shiny = false, level = 100, ability = '', friendship, teraType, nature = '', ivs: Record<string, number> = {}, moves: string[] = [], item;
  for (let i = 1; i < lines.length; i++) {
    const line = lines[i];
    if (line.startsWith('Ability:')) ability = line.replace('Ability:', '').trim();
    else if (line.startsWith('Level:')) level = parseInt(line.replace('Level:', '').trim());
    else if (line.startsWith('Shiny:')) shiny = line.includes('Yes');
    else if (line.startsWith('Friendship:')) friendship = parseInt(line.replace('Friendship:', '').trim());
      ...
      for (const pair of ivPairs) {
        ...
        if (stat) ivs[stat] = parseInt(val);
      }
    } else if (line.startsWith('- ')) moves.push(line.replace('- ', ''));
    else if (line.endsWith('(M)') || line.endsWith('(F)')) gender = line.endsWith('(M)') ? 'M' : 'F';
  }
  const pokemon = {
    name,
    gender,
...
export function isSelectablePokemon(name: string, pokemonList: Array<{ name: string }>): boolean {
...
export function applyBasePokemonEdit(base: BasePokemon, edit: Partial<BasePokemon>): BasePokemon {
  const result = { ...base };
  if (BASEPOKEMON_EDIT_RULES.allowLevelUp && edit.level && edit.level > base.level) result.level = edit.level;
  if (BASEPOKEMON_EDIT_RULES.allowMovesEdit && edit.moves) result.moves = edit.moves;
  if (BASEPOKEMON_EDIT_RULES.allowItemEdit && edit.item) result.item = edit.item;
  if (BASEPOKEMON_EDIT_RULES.allowAbilityEdit && edit.ability) result.ability = edit.ability;
  if (BASEPOKEMON_EDIT_RULES.allowNatureEdit && edit.nature) result.nature = edit.nature;
